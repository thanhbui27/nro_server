...
public class DaiHoiVoThuatService {
    ...
    public void Update() {
        ...
        daihoi.listReg.stream().filter((me) -> (me != null && me.zone.map.mapId != 52 && me.zone.map.mapId != 51 && DaiHoiVoThuat.gI().Minutes >= daihoi.min_start_temp)).map((me) -> {
            Service.getInstance().sendThongBao(me, "Bạn đã bị tước quyền thi đấu do không có mặt kịp giờ");
            return me;
        }).forEachOrdered((me) -> {
        ...
        if (DaiHoiVoThuat.gI().Second == 0 && DaiHoiVoThuat.gI().Minutes < daihoi.min_start_temp) {
            if (daihoi.listReg.size() > 1) {
                Service.getInstance().sendThongBao(daihoi.listReg, "Vòng " + daihoi.round + " sẽ bắt đầu sau " + (daihoi.min_start_temp - DaiHoiVoThuat.gI().Minutes) + " phút nữa");
            }
            ...
            if (daihoi.listReg.size() > 1) {
                if (daihoi.listReg.size() % 2 == 0) {
                    ...
                    if(pl != null){
                        ...
                        Service.getInstance().sendThongBao(pl, "Chúc mừng bạn đã may mắn lọt vào vòng trong");
                    }
                    MatchDHVT();
                }
            }else if(daihoi.listReg.size() == 1 && daihoi.listPlayerWait.isEmpty() && countNull >= maxZone){
                Service.getInstance().sendThongBao(daihoi.listReg.get(0), "Bạn đã vô địch giải " + daihoi.NameCup);
                daihoi.round = 1;
    ...
    public void Reg(Player player) {
        ...
        if (daihoi.gem > 0) {
            if (player.inventory.ruby >= daihoi.gem) {
                ...
                Service.getInstance().sendThongBao(player, "Bạn Không Đủ Ngọc Để Đăng Ký");
                isReg = false;
            }
            ...
            if (player.inventory.gold >= daihoi.gold) {
                ...
                Service.getInstance().sendThongBao(player, "Bạn Không Đủ Vàng Để Đăng Ký");
                isReg = false;
            }
        } else {
            Service.getInstance().sendThongBao(player, "Bạn Không Thể Đăng Ký Giải Đấu Này");
        }
        if (isReg) {
            if(player.isPl()){
                daihoi.listReg.add(player);
                Service.getInstance().sendMoney(player);
                Service.getInstance().sendThongBao(player, "Bạn đã đăng ký thành công!Vui lòng không rời khỏi đại hội võ thuật để tránh bị tước quyền thi đấu!!");
            }
        }
    }
}
