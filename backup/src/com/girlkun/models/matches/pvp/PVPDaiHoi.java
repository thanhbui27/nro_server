    ...
    public void update() {
        if(Util.canDoWithTime(lastTimePVP, 180000)){
            if(p1.nPoint.hp > p2.nPoint.hp){
                ...
                Service.getInstance().sendThongBao(p1, "Trận đấu hòa. Bạn nhận được " + Util.numberToMoney(this.goldReward) + " vàng");
                p1.inventory.gold += this.goldReward / 2;
                Service.getInstance().sendMoney(p1);
                dh.listReg.add(p1);
                p1.lastTimeWin = System.currentTimeMillis();
                p1.isWin = true;
                
                Service.getInstance().sendThongBao(p2, "Trận đấu hòa. Bạn nhận được " + Util.numberToMoney(this.goldReward) + " vàng");
                p2.inventory.gold += this.goldReward / 2;
                Service.getInstance().sendMoney(p2);
                dh.listReg.add(p2);
