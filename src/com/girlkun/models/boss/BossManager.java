...
public class BossManager implements Runnable {
    ...
    public void showListBoss(Player player) {
        ...
        try {
            ...
            msg.writer().writeByte((int) bosses.stream()
    ...
    public synchronized void callBoss(Player player, int mapId) {
        try {
            if (BossManager.gI().existBossOnPlayer(player)
                    || player.zone.items.stream().anyMatch(
                            itemMap -> ItemMapService.gI().isBlackBall(itemMap.itemTemplate.id))
    ...
    public Boss getBossById(int bossId) {
        return BossManager.gI().bosses.stream().filter(boss -> boss.id == bossId && !boss.isDie())
